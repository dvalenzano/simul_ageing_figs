...
# This generates the 60kth stage survival-reproduction plot for small, large, and very large populations, in the 
# constant resources condition and in the pop resource condition (ll is only present for const)
...
Jlpr_as = float(lpl_as.repr_rate_junk_in[-1])/0.4
Jlps_as = (float(lpl_as.surv_rate_junk_in[-1])-0.98)/0.02
Jscr_as = float(scl_as.repr_rate_junk_in[-1])/0.4
Jscs_as = (float(scl_as.surv_rate_junk_in[-1])-0.98)/0.02
Jspr_as = float(spl_as.repr_rate_junk_in[-1])/0.4
Jsps_as = (float(spl_as.surv_rate_junk_in[-1])-0.98)/0.02
...
Jlp_as = ((str(Jlps_as)+',')*71+(str(Jlpr_as)+',')*55).split(',')
Jsc_as = ((str(Jscs_as)+',')*71+(str(Jscr_as)+',')*55).split(',')
Jsp_as = ((str(Jsps_as)+',')*71+(str(Jspr_as)+',')*55).split(',')
Jllc_as = ((str(Jllcs_as)+',')*71+(str(Jllcr_as)+',')*55).split(',')

Jnk_s_as = 'Jlc_as,Jlp_as,Jsc_as,Jsp_as,Jllc_as\n'+','.join([','.join(list(i))+'\n' for i in zip(Jlc_as,Jlp_as,Jsc_as,Jsp_as,Jllc_as)]).replace('\n,','\n')
